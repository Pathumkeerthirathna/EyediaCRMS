...
//meetings 
//add 
export const AddMeetings = ({prospectID, onClose, onMeetingAdded, editMeeting}) =>{
    ...
    const {
        ...
        useEffect(() => {  
            ...
            if (editMeeting)Object.entries(editMeeting).forEach(([key, value])=>
            {
              if(defaultValues.hasOwnProperty(key)){
                setValue(key, value);
              }
            });
        }, [prospectID, editMeeting, setValue]); //set prospectId and set data to the form when editing
    ...
    const onSubmit = async (data) => {
      ...
      if(editMeeting && !editMeeting.id){
        ...
        try{
          if(editMeeting){
            //edit meetings
            await UpdateMeetings.updateProspectMeeting(meetingData);
            alert("meeting updated successfully!");
          }else{
            //add new
            await axios.post(
              `${apiUrl}/api/Meeting/AddProspectMeeting`, meetingData, {
                prospectID: data.prospectID,
              },{
...
useEffect (() => {
  const getProspectMeetingLogs = async () => {
    try{
  ...
  getProspectMeetingLogs();
})
};

//update 
export const UpdateMeetings = {
 updateProspectMeeting: async (meetingData) => {
  
  console.log("Updating meeting data:", meetingData);

  try {
    const response = await axios.put(`${apiUrl}/api/Meeting/UpdateMeeting`, meetingData,{
      headers: {"Content-Type" : "application/json"},
    });
    return response.data;

  }catch (error){
    console.error("error updating prospect meeting:", error);
    throw error;
  }
 },
};
...
export const UpdateDecisions = {
  updateMeetingDecisions: async (decisionData) => {
